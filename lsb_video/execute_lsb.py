...
def main():
    ...
    call(["ffmpeg", "-i", "tmp/%d.png" , "-vcodec", "png","-pix_fmt", "yuv420p", "tmp/video.mov", "-y"],stdout=open(os.devnull, "w"), stderr=STDOUT)
    
    call(["ffmpeg", "-i", "tmp/video.mov", "-i", "tmp/audio.mp3","-pix_fmt", "yuv420p", "-codec", "copy", "video.mov", "-y"],stdout=open(os.devnull, "w"), stderr=STDOUT)
    clean_tmp()
if __name__ == "__main__":
    while True:
        ...
        if choice == '1':
            ...
            decode_string(input("Enter the name of encrypted video file along with the extension: "))
        else:
            break